/* Repository Explorer — repository-explorer plugin (ClankerSkills)
 * Static, dependency-light Markdown browser + review/commenting layer.
 * - Manifest (manifest.json) lists files; bodies are fetched lazily over HTTP.
 * - Comments persist in localStorage, anchored to heading slugs or selections.
 ...
 *
 * Everything repo-specific lives in window.EXPLORER_CONFIG (set in index.html):
...
(function () {
  ...
  function onDocMouseUp() {
    setTimeout(function () {
      ...
      if (!sel || sel.isCollapsed || !state.current) {
        el.selectionBtn.hidden = true;
        return;
      }
      var text = sel.toString().trim();
      if (!text || !el.doc.contains(sel.anchorNode)) {
        el.selectionBtn.hidden = true;
        return;
      }
      ...
      el.selectionBtn.style.top = window.scrollY + rect.top - 38 + "px";
      el.selectionBtn.style.left = window.scrollX + rect.left + rect.width / 2 - 45 + "px";
      el.selectionBtn._payload = {
        quote: text.length > 280 ? text.slice(0, 277) + "…" : text,
        anchor: nearestHeading(sel.anchorNode),
      };
    }, 1);
  }
  ...
  function wireChrome() {
    ...
    el.doc.addEventListener("mouseup", onDocMouseUp);
